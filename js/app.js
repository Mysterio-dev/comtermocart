 ...
 */
    ...
    (() => {
        ...
        function updateDeliveryInfo(selectedValue) {
            ...
            const commonValues = {
                moscowDelivery: {
                    method: "Доставка по Москве",
                    cost: "500 руб."
                },
                pickupDelivery: {
                    method: "Пункт выдачи Comtermo",
                    cost: "Бесплатно"
                },
                transportDelivery: {
                    method: "Доставка ТК",
                    cost: "500 руб + (рассчитывается компанией)"
                },
                areaDelivery: {
                    method: "Доставка в область",
                    cost: "0 руб."
                }
            };
            const deliveryOptions = {
                ...commonValues,
                ...Object.fromEntries(Object.entries(commonValues).map((([key, value]) => [ `${key}Two`, value ])))
            };
        ...
        document.addEventListener("DOMContentLoaded", (function() {
            let totalQuantity = 0;
            let generalPrice = 0;
            const productItems = document.querySelectorAll(".c-cart__order-item");
            ...
            function updateTotalPrice() {
                ...
                productItems.forEach((function(productItem) {
                    ...
                    console.log(`Цена товара: ${pricePerUnit}, Количество: ${quantity}, Общая стоимость за товар: ${totalPriceForItem}`);
                }));
                console.log("Обновлена общая цена:", totalPrice);
                generalPrice = totalPrice;
                cartInfoPrice.textContent = generalPrice.toLocaleString();
                totalPriceCountUp.update(generalPrice);
                console.log("Обновлена общая цена:", generalPrice);
            }
            productItems.forEach((function(productItem) {
                const quantityInput = productItem.querySelector(".quantity__input input");
                ...
                quantityInput.addEventListener("input", (function() {
                    ...
                    if (isNaN(quantity) || quantity < 1) quantity = 1; else if (quantity > 9999) quantity = 9999;
            ...
            function handleDeleteButtonClick(event) {
                console.log("Клик по кнопке удаления товара");
                let button = event.target;
                let listItem = button.closest(".c-cart__order-item");
                if (listItem) {
                    let quantityElement = listItem.querySelector(".quantity__input input");
                    let quantity = parseInt(quantityElement.value);
                    totalQuantity -= quantity;
                    console.log("Общее количество товаров после удаления:", totalQuantity);
                    updateCounters();
                    listItem.remove();
                    let remainingItems = document.querySelectorAll(".c-cart__order-item");
                    if (remainingItems.length === 0) updateEmptyCart();
                    toastify({
                    ...
                    updateTotalPrice();
                    console.log("Общая цена после удаления:", generalPrice);
                }
            }
            updateTotalPrice();
                ...
                distanceInputs.forEach((function(distanceInput) {
                    distanceInput.addEventListener("input", (function() {
                        ...
                        if (enteredDistance === "") deliveryInfoBlocks.forEach((function(deliveryInfoBlock) {
                            ...
                            } else deliveryInfoBlocks.forEach((function(deliveryInfoBlock) {
                                deliveryInfoBlock.textContent = "0 руб.";
                                toastify({
